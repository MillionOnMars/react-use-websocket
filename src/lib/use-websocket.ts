...
export const useBaseWebsocket = <T = unknown>(
  url: string | (() => string | Promise<string>) | null,
  options: Options = DEFAULT_OPTIONS,
  connect: boolean = true,
): Omit<WebSocketHook<T>, "lastMessage" | "lastJsonMessage"> => {
  ...
  const setLastMessage = options.lastMessageStore?.[1];
...
export const useWebSocket = <T = unknown>(
  ...
  const lastMessageStore = useState<WebSocketEventMap["message"] | null>(null);
  const [lastMessage] = lastMessageStore;
  ...
  const { sendMessage, readyState, sendJsonMessage, getWebSocket } =
    useBaseWebsocket<T>(
      url,
      { ...options, lastMessageStore: lastMessageStore },
      connect,
    );
